...
const WaButton = ({ product, style, text }) => {
    ...
    return (
        <Link
            href={`${product ? `https://wa.me/${phone}?text=${encodeURIComponent(message)}` : `https://wa.me/${phone}`}`}
            className={`${style ? style : 'block w-[11rem] h-16 mt-20 text-white bg-[#25D366]'}`}>
